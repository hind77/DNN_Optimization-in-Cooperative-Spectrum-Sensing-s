...
class DNNModelP2(DNNModel):
    ...
    def choose_model_P2(init_model, choice: str):
        ...
        if choice == "dropout_Rl1":           
                ...
                #model.add(layers.Dropout(0.2))
                #model.add(BatchNormalization())
                model.add(Dense(64, kernel_regularizer=regularizers.l1(0.00001)))
                model.add(LeakyReLU(alpha=0.5))
                #model.add(layers.Dropout(0.2))
                #model.add(BatchNormalization())
                model.add(Dense(32, kernel_regularizer=regularizers.l1(0.00001)))
                model.add(LeakyReLU(alpha=0.5))
                #model.add(layers.Dropout(0.2))
                #model.add(BatchNormalization())
                model.add(Dense(16, activation='relu', kernel_regularizer=regularizers.l1(0.00001)))
