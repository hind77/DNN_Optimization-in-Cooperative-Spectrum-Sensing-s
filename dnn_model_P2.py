...
class DNNModelP2(DNNModel):
    ...
    def loss_fn_P2(cls,snrs: np.ndarray, predicted) -> float:
      ...
      #C_np = np.diag(1+2*snrs_np)
      print("c matrix", C.shape)
      C_inv = tf.linalg.inv(C)
      #C_inv_np = np.linalg.inv(C_np)
      ...
      #exp4 = tf.math.sqrt(tf.math.multiply(tf.math.multiply(tf.math.multiply(2*fs*(T_cte - num_sens*tr),tf.keras.backend.transpose(snrs_tf)),tf.keras.backend.transpose(C_inv)),snrs_tf))
      #exp4 = np.dot(np.dot(np.dot((2*fs*(T_cte - num_sens*tr)), snrs_np.transpose()), C_inv_np.transpose()), snrs_np)
